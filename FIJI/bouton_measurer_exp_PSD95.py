...
- convert the PSD95 channel to a mask (int treshold is used to exclude background)
...
for dir in dirs:
	...
	for num_image in indices:
		...
		#PSD95
		PSD = [x for x in list if "C3-" in x and num_pattern in x]
		...
		assert len(ACT) == 1 and len(PSD) == 1, num_image
	...
	output_file = os.path.join(dir, 'final_table_PSD95.csv')
