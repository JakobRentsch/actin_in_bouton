...
'''
-this script runs over the directories of the preprocessed expansion data (split channel) for all conditions and 
preparations to find the final_table_PSD95.csv files containing the measurement outputs (FIJI macros: 
bouton_measurer_exp_VGLUT1.py and full_image_actin_measurer_exp.py) of all rois and the whole actin images 
...
-the saved .csv files contain indentifiers columns for each condition (WT = 1, TKO = 2)
...
all_conditions_area_um^2 --> area of rois in nm^2 (as defined by bouton_measurer.py FIJI macro)
                            summarized for all conditions                  
-script also generated quick boxplot to visualize output, these are not saved
'''

input_directory = '/Users/jakobrentsch/FU Box/Papers/Dual color sted of actin and syp/Expansion data/20241120_Data_for_jakob/Channel Split'
output_directory = '/Users/jakobrentsch/FU Box/Papers/Dual color sted of actin and syp/Expansion data/20241120_Data_for_jakob/Channel Split/Source Data/VGLUT1'
conditions = ['WT', 'TKO']
...
def csv_files_summarizer(directoy, condition):
    ...
    for root, dirs, files in os.walk(input_directory):
        for file in files:
            ...
            if file.endswith("final_table_VGLUT1.csv") and condition in root:
...
np.savetxt(output_directory + '/all_conditions_area_um^2_rois.csv', all_condis_rois_area_header, fmt='%s',
           delimiter=',')
...
def boxplotter(array, figure_n, figure_title, ylabel):
    ...
    plt.boxplot([array[~np.isnan(array[:, 0]), 0],
                ...
                labels=conditions, showmeans=True, showfliers=False)
